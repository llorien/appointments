...
birch_ns( 'birchschedule.aportal', function( $ns ) {
		...
		birch_defn( $ns, 'wp_init', function() use( $ns, $_ns_data, $birchschedule ) {
				...
				add_filter( 'birchschedule_enotification_get_appointment1on1_merge_values',
		...
		birch_defn( $ns, 'enqueue_scripts_sc_cancel_appointment', function() use( $birchschedule ) {
		...
		birch_defn( $ns, 'enqueue_scripts_sc_reschedule_appointment', function() use( $birchschedule ) {
				$birchschedule->view->register_3rd_scripts();
				$birchschedule->view->register_3rd_styles();
		...
		birch_defn( $ns, 'add_shortcode', function( $shortcodes ) use( $_ns_data ) {
				$shortcodes[] = $_ns_data->SC_CANCEL_APPOINTMENT;
				$shortcodes[] = $_ns_data->SC_RESCHEDULE_APPOINTMENT;
				return $shortcodes;
			} );

		birch_defn( $ns, 'add_merge_values', function( $appointment1on1, $appointment1on1_id )
		...
		birch_defn( $ns, 'render_reschedule_pages_settings', function() use( $ns, $birchschedule, $_ns_data ) {
				global $birchpress;
...
?>
		...
		birch_defn( $ns, 'get_time_before_cancel', function() use( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'get_time_before_reschedule', function() use( $ns, $birchschedule ) {
				$booking_preferences = $birchschedule->bpreferences->get_booking_preferences();
				$time_before_reschedule = $booking_preferences['time_before_reschedule'];
				return $time_before_reschedule;
			} );

		birch_defn( $ns, 'get_times_before_cancel', function() use( $birchschedule ) {
				return $birchschedule->bpreferences->get_cut_off_times();
			} );

		birch_defn( $ns, 'get_times_before_reschedule', function() use( $birchschedule ) {
				return $birchschedule->bpreferences->get_cut_off_times();
			} );

		birch_defn( $ns, 'get_time_before_cancel_html', function() use( $ns, $birchschedule ) {
				global $birchpress;
...
?>
		...
		birch_defn( $ns, 'get_time_before_reschedule_html', function() use( $ns, $birchschedule ) {
				global $birchpress;
...
?>
		...
		birch_defn( $ns, 'render_rescheduling_policies', function() use( $ns, $birchschedule ) {
?>
        <ul style="margin: 0;">
            <li>
		...
		birch_defn( $ns, 'encode_appointment1on1_id', function( $appointment1on1_id ) use( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'decode_appointment1on1_id', function( $apt_key ) use( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'get_sc_cancel_appointment_nonexist', function() use( $ns, $birchschedule ) {
...
?>
		...
		birch_defn( $ns, 'get_sc_cancel_appointment_styles', function() {
				ob_start();
?>
		<style type="text/css">
		...
		birch_defn( $ns, 'get_sc_cancel_appointment_confirm', function( $appointment1on1 )
...
?>
		...
		birch_defn( $ns, 'get_sc_cancel_appointment_success', function() use( $ns, $birchschedule ) {
				$i18n_messages = $birchschedule->view->get_frontend_i18n_messages();
				ob_start();
?>
		<p>
		...
		birch_defn( $ns, 'get_sc_cancel_appointment_outoftime', function() use( $ns, $birchschedule ) {
				$i18n_messages = $birchschedule->view->get_frontend_i18n_messages();
				ob_start();
?>
		<p>
		...
		birch_defn( $ns, 'get_sc_cancel_appointment_content', function( $attr ) use( $ns, $birchschedule ) {
				global $birchpress;
		...
		birch_defn( $ns, 'ajax_cancel_appointment', function() use( $ns, $birchschedule ) {
				global $birchpress;
		...
		birch_defn( $ns, 'get_sc_reschedule_appointment_styles', function() use( $ns, $birchschedule ) {
				ob_start();
?>
		<style type="text/css">
		...
		birch_defn( $ns, 'remove_current_appointment', function( $appointments ) use( $ns, $_ns_data ) {
		...
		birch_defn( $ns, 'get_avaliable_time_options', function( $staff_id, $location_id, $service_id, $date )
			use( $ns, $birchschedule, $_ns_data ) {
		...
		birch_defn( $ns, 'ajax_get_avaliable_time_options', function() use( $ns, $birchschedule, $_ns_data ) {
				global $birchpress;
		...
		birch_defn( $ns, 'get_sc_reschedule_appointment_nonexist', function() use( $ns ) {
				return $ns->get_sc_cancel_appointment_nonexist();
			} );

		birch_defn( $ns, 'get_sc_reschedule_appointment_success', function() use( $ns, $birchschedule ) {
				$i18n_messages = $birchschedule->view->get_frontend_i18n_messages();
				ob_start();
?>
		<p>
		...
		birch_defn( $ns, 'get_sc_reschedule_appointment_confirm', function( $appointment1on1 ) use( $ns, $birchschedule, $_ns_data ) {
				global $birchpress;
				...
				$time_options = $ns->get_avaliable_time_options( $appointment1on1['_birs_appointment_staff'],
					$appointment1on1['_birs_appointment_location'], $appointment1on1['_birs_appointment_service'], $date_0 );
				ob_start();
...
?>
			...
			<div id='birs_appointment_form'>
				<ul>
		            ...
		            <li class="birs_form_field birs_appointment_date">
		                ...
		                <div class="birs_field_content">
			                <input id="birs_appointment_datepicker" type="text" name="birs_appointment_datepicker">
		...
		birch_defn( $ns, 'get_sc_reschedule_appointment_outoftime', function() use( $ns, $birchschedule ) {
				$i18n_messages = $birchschedule->view->get_frontend_i18n_messages();
				ob_start();
?>
		<p>
		...
		birch_defn( $ns, 'get_sc_reschedule_appointment_content', function( $attr ) use( $ns, $birchschedule, $_ns_data ) {
				global $birchpress;
		...
		birch_defn( $ns, 'get_appointment1on1_merge_values', function( $appointment1on1_id ) use( $ns, $birchschedule ) {
				return $birchschedule->model->mergefields->get_appointment1on1_merge_values( $appointment1on1_id );
			} );

		birch_defn( $ns, 'validate_reschedule_info', function() use( $ns ) {
				$errors = array();
		...
		birch_defn( $ns, 'ajax_reschedule_appointment', function() use( $ns, $birchschedule, $_ns_data ) {
				global $birchpress;
