...
birch_ns( 'birchschedule.acontrol', function( $ns ) {
		...
		birch_defn( $ns, 'filter_locations_listing_order', function( $order ) use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'filter_services_listing_order', function( $order ) use( $ns, $birchschedule ) {
				if ( current_user_can( 'edit_birs_appointments' ) &&
					current_user_can( 'edit_others_birs_appointments' ) ) {
		...
		birch_defn( $ns, 'filter_staff_listing_order', function( $order ) use( $ns, $birchschedule ) {
				if ( current_user_can( 'edit_birs_appointments' ) &&
					current_user_can( 'edit_others_birs_appointments' ) ) {
		...
		birch_defn( $ns, 'filter_view_appointments_edit_get_action_cancel_html', function( $html ) {
		...
		birch_defn( $ns, 'filter_view_appointments_edit_clientlist_get_item_actions', function( $actions, $item ) {
				if ( !current_user_can( 'delete_birs_appointments' ) ||
					!current_user_can( 'delete_published_birs_appointments' ) ) {
		...
		birch_defn( $ns, 'filter_others_for_author', function( $query ) use( $ns, $birchschedule ) {
				global $pagenow;
		...
		birch_defn( $ns, 'set_appointments_authors', function( $post ) use( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'update_staff_settings', function( $user_id ) use( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'init_roles', function() use( $ns, $birchschedule ) {
