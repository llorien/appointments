...
birch_ns( 'birchschedule.fdadmin', function( $ns ) {
		...
		birch_defn( $ns, 'add_filter_avaliable_times_checkbox', function( $html ) use( $ns, $birchschedule ) {
				ob_start();
?>
        <ul>
		...
		birch_defn( $ns, 'ajax_get_available_time_options', function() use( $ns, $birchschedule ) {
				global $birchpress;
...
?>
		...
		birch_defn( $ns, 'add_scheduled_time',
			function( $time_options, $staff_id, $location_id, $service_id, $date )
		...
		birch_defn( $ns, 'ajax_get_available_reschedule_time_options', function() use ( $ns ) {
