...
birch_ns('birchschedule.pintegration', function($ns){
		...
		birch_defn( $ns, 'add_payment_type', function($payment_types) {
				$payment_types['paypal'] = 'PayPal';
				return $payment_types;
			});

		birch_defn( $ns, 'add_prepayment_method', function($methods) {
		...
		birch_defn( $ns, 'handle_paypal_ipn', function() use($ns, $birchschedule) {
				...
				if($fp) {
					...
					while (!feof($fp)) {
						...
						if (strcmp($res, "VERIFIED") == 0) {
							...
							$paypal_email = trim($ns->get_paypal_email());
		...
		birch_defn( $ns, 'get_paypal_button_html', function($post_data) use($ns, $birchschedule) {
				ob_start();
?>
		...
		birch_defn( $ns, 'get_bookingform_script', function() {
				ob_start();
?>
        <script type="text/javascript">
            (function($){
                ...
                var ns = namespace('birchschedule.pintegration.bookingform');

                defineFunction(ns, 'getPlaceOrderFuncs', function(funcs) {
                ...
                addAction('birchschedule.ppayment.bookingform.initBefore', function() {
                    addFilter('birchschedule.ppayment.bookingform.getPlaceOrderFuncs', ns.getPlaceOrderFuncs);
                });
		...
		birch_defn( $ns, 'get_shortcode_page_url', function() {
				$shortcode_page_url = $_POST['birs_shortcode_page_url'];
				return $shortcode_page_url;
			});

		birch_defn( $ns, 'is_paypal_enabled', function() {
		...
		birch_defn( $ns, 'get_paypal_email', function() {
				$options = get_option('birchschedule_options_payments', array());
				$paypal_email = "";
				if(isset($options['paypal'])) {
		...
		birch_defn( $ns, 'get_return_url', function($arg) use($ns, $birchschedule) {
				$shortcode_page_url = $ns->get_shortcode_page_url();
				$thankyou_url = add_query_arg($arg, $shortcode_page_url);
				return $thankyou_url;
			});

		birch_defn( $ns, 'get_cancel_return_url', function() use($ns, $birchschedule) {
				return $ns->get_shortcode_page_url();
			});

		birch_defn( $ns, 'get_notify_url', function() {
				return home_url() . '/index.php?bpscheduler_listener=paypal_ipn';
			});

		birch_defn( $ns, 'get_prepayment_method_html',
			function($html, $method_name, $method, $appointment1on1_id) use($ns, $birchschedule) {

				if($method_name !== 'paypal') {
					return $html;
				}
				$enabled = $ns->is_paypal_enabled();
				$paypal_email = $ns->get_paypal_email();
				if(!$enabled) {
					return $html;
				}
				$appointment1on1 =
		...
		birch_defn( $ns, 'render_settings_paypal', function() use($ns, $birchschedule) {
				$enabled = $ns->is_paypal_enabled();
				$paypal_email = esc_attr($ns->get_paypal_email());
				$enabled_check = '';
				if($enabled) {
