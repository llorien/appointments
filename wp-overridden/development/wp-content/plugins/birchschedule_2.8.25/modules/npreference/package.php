...
birch_ns( 'birchschedule.npreference', function( $ns ) {
		...
		birch_defn( $ns, 'is_no_preference_enabled', function() {
				return true;
			} );

		birch_defn( $ns, 'add_staff_no_preference', function( $staff_order ) {
				array_unshift( $staff_order, -1 );
				return $staff_order;
			} );

		birch_defn( $ns, 'add_wc_staff_options_no_preference', function( $staff_options ) use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'add_staff_np_locations_map', function( $map ) use ( $ns, $birchschedule ) {
				$i18n_messages = $birchschedule->view->get_frontend_i18n_messages();
				$new_map = array();
		...
		birch_defn( $ns, 'add_staff_np_services_map', function( $map ) use( $ns, $birchschedule ) {
				$i18n_messages = $birchschedule->view->get_frontend_i18n_messages();
				$new_map = array();
		...
		birch_defn( $ns, 'get_staff_avaliable_time',
		...
		birch_defn( $ns, 'select_staff_from_alternatives', function() use( $ns ) {
		...
		birch_defn( $ns, 'random_staff_listing_order', function( $order ) use( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'get_last_appointment1on1', function() {
				$appointment1on1_id = get_option( 'birs_last_appointment1on1' );
				return $appointment1on1_id;
			} );

		birch_defn( $ns, 'change_staff_order_in_turn', function( $order ) use( $ns, $birchschedule ) {
				$order_type = $birchschedule->model->get_staff_listing_order_type();
		...
		birch_defn( $ns, 'record_last_appointment1on1', function( $appointment1on1_id, $new_status, $old_status ) {
