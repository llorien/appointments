...
birch_ns( 'birchschedule.gbooking', function( $ns ) {
		...
		birch_defn( $ns, 'wp_init', function() {
		...
		birch_defn( $ns, 'enqueue_scripts', function() use ( $birchschedule ) {
				$birchschedule->view->enqueue_scripts(
					array( 'birchschedule_gbooking' )
				);
			} );

		birch_defn( $ns, 'ajax_change_appointment_capacity', function() use ( $ns, $birchschedule ) {
				$appointment = array(
		...
		birch_defn( $ns, 'ajax_add_client', function() use ( $ns, $birchschedule ) {
				global $birchpress;
		...
		birch_defn( $ns, 'save_service_capacity', function( $post ) use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'get_service_capacity', function( $capacity, $service_id ) use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'render_gbooking_settings', function( $post ) use ( $ns, $birchschedule ) {
...
?>
		...
		birch_defn( $ns, 'render_change_appointment_capacity', function( $post ) use ( $ns, $birchschedule ) {
...
?>
		...
		birch_defn( $ns, 'render_actions_add_client', function() use ($ns){
...
?>
		...
		birch_defn( $ns, 'get_client_info_html', function() use ( $birchschedule ) {
				return $birchschedule->view->appointments->edit->clientlist->edit->get_client_info_html( 0 );
			} );

		birch_defn( $ns, 'get_appointment1on1_info_html', function() use ( $birchschedule ) {
				return $birchschedule->view->appointments->edit->clientlist->edit->get_appointment1on1_info_html( 0, 0 );
			} );


		birch_defn( $ns, 'get_add_client_html', function() use ( $ns, $birchschedule ) {
				ob_start();
...
?>
		...
		birch_defn( $ns, 'validate_client_info', function() use ( $birchschedule ) {
				return $birchschedule->view->appointments->new->validate_client_info();
			} );

		birch_defn( $ns, 'validate_appointment1on1_info', function() use ( $birchschedule ) {
				return $birchschedule->view->appointments->new->validate_appointment1on1_info();
			} );

		birch_defn( $ns, 'add_client', function() use ( $ns, $birchschedule ) {
				global $birchpress;
