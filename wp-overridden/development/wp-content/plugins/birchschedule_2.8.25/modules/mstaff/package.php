...
birch_ns( 'birchschedule.mstaff', function( $ns ) {
		...
		birch_defn( $ns, 'init', function() use ( $ns, $_ns_data ) {
				...
				$_ns_data->default_calendar_color = '#3366CC';
		...
		birch_defn( $ns, 'redefine_functions', function() use ( $ns, $birchschedule ) {
				...
				birch_defn( $birchschedule->view->bookingform, 'get_shortcode_attrs', $ns->get_sc_bookingform_attrs );
			} );
		...
		birch_defn( $ns, 'explode_ids', function( $ids ) use ( $ns ) {
				if ( !$ids ) {
					return false;
				}
				$ids = explode( ',', $ids );
		...
		birch_defn( $ns, 'get_sc_bookingform_attrs', function( $attr ) use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'enqueue_scripts', function( $hook ) use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'add_scripts', function() use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'add_styles', function() use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'add_all_staff', function( $map ) use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'add_all_staff_listing_order', function( $map ) {
				return array_merge( array( -1 ), $map );
			} );

		birch_defn( $ns, 'get_staff_color_map', function() use ( $ns, $birchschedule, $_ns_data ) {
		...
		birch_defn( $ns, 'add_meta_boxes', function() use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'save_staff_data', function( $post ) use ( $ns ) {
		...
		birch_defn( $ns, 'render_staff_color', function( $post ) use ( $ns, $birchschedule, $_ns_data ) {
...
?>
		...
		birch_defn( $ns, 'add_appointments_color', function( $appointments ) use ( $ns, $birchschedule, $_ns_data ) {
