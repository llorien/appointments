...
birch_ns( 'birchschedule.cintegration', function( $ns ) {
		...
		birch_defn( $ns, 'if_sync_past_events', function() use ( $ns ) {
				return false;
			} );

		birch_defn( $ns, 'force_sync', function() use ( $ns ) {
				wp_remote_post(
					site_url( '/?birs_sync_icals=yes' ),
					array(
						'timeout' => 0.01,
						'blocking' => false,
		...
		birch_defn( $ns, 'wp_init', function() use( $ns ) {
				...
				$sync_icals_request = preg_match( '/\?birs_sync_icals=/', $_SERVER['REQUEST_URI'] );
				if ( $sync_icals_request ) {
					$ns->sync_icals_by_request();
					die;
				}
				...
				add_action( 'birchschedule_cintegration_sync_icals_action', array( $ns, 'sync_icals' ) );
				add_filter( 'cron_schedules', array( $ns, 'add_sync_icals_interval' ) );
			} );

		birch_defn( $ns, 'is_icalendar_url', function( $url ) use( $ns ) {
		...
		birch_defn( $ns, 'get_icalendar_id', function( $url ) use( $ns ) {
				$ical_id = '';
		...
		birch_defn( $ns, 'schedule_sync_icals', function() use( $ns ) {
				if ( !wp_next_scheduled( 'birchschedule_cintegration_sync_icals_action' ) ) {
					wp_schedule_event( time(), 'birs_sync_icals_interval',
						'birchschedule_cintegration_sync_icals_action' );
				}
			} );

		birch_defn( $ns, 'add_sync_icals_interval', function( $schedules ) use( $ns ) {
		...
		birch_defn( $ns, 'sync_icals_by_request', function() use( $ns ) {
				if ( isset( $_GET['birs_sync_icals'] ) ) {
					$ns->sync_icals();
				}
			} );

		birch_defn( $ns, 'get_ical_content', function( $import_url ) use( $ns ) {
		...
		birch_defn( $ns, 'sync_icals', function() use( $ns, $birchschedule ) {
				global $birchpress;
		...
		birch_defn( $ns, 'post_get_staff', function( $staff ) use( $ns ) {
		...
		birch_defn( $ns, 'pre_save_staff', function( $staff, $staff_orig, $config ) use( $ns ) {
		...
		birch_defn( $ns, 'ajax_new_calendar_import_url', function() use( $ns ) {
				$new_import_url_id = uniqid( rand() );
				$ns->render_import_url( $new_import_url_id, "" );
				die;
			} );

		birch_defn( $ns, 'add_imported_events',
			function( $appointments, $start, $end, $location_id, $staff_id ) use ( $ns, $birchschedule ) {
				global $birchpress;
		...
		birch_defn( $ns, 'if_imported_event_affect', function( $imported_appointment, $start, $end ) {
				$imported_start = $imported_appointment['start'];
				$imported_end = $imported_appointment['end'];
		...
		birch_defn( $ns, 'add_imported_days_off', function( $days_off, $staff_id ) use( $ns ) {
				global $birchpress;
		...
		birch_defn( $ns, 'add_imported_busy_events',
			function( $busy_times, $staff_id, $location_id, $date ) use ( $ns ) {
				global $birchpress;
		...
		birch_defn( $ns, 'is_vevent_free', function( $e ) {
				return isset( $e->TRANSP ) && strtoupper( $e->TRANSP ) == 'TRANSPARENT';
			} );

		birch_defn( $ns, 'get_vevent_title', function( $e ) {
				return __( 'BLOCKED', 'birchschedule' );
			} );

		birch_defn( $ns, 'convert_vevent_to_appointment', function( $e, $timezone, $staff ) use( $ns, $_ns_data ) {
		...
		birch_defn( $ns, 'figure_out_timezone', function( $vcalendar ) use( $ns ) {
				global $birchpress;
		...
		birch_defn( $ns, 'convert_ical_to_appointments',
			function( $ical_content, $staff ) use( $ns ) {
		...
		birch_defn( $ns, 'get_imported_appointments',
			function( $staff_id, $start = null, $end = null ) use( $ns, $birchschedule ) {
				global $birchpress;
		...
		birch_defn( $ns, 'get_export_appointments_time_start', function() {
				return time() - 60 * 60 * 24 * 30;
			} );

		birch_defn( $ns, 'get_export_appointments_time_end', function() {
				return time() + 60 * 60 * 24 * 365;
			} );

		birch_defn( $ns, 'render_icalendar', function() use( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'generate_ical_id', function( $staff_id ) {
				return uniqid( $staff_id );
			} );

		birch_defn( $ns, 'save_staff_data', function( $post ) use( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'get_icalendar_url', function( $ical_id ) {
				return home_url() . '/index.php/birchpress/scheduler/icalendar/' . urlencode( $ical_id );
			} );

		birch_defn( $ns, 'render_staff_calendar_integration', function( $post ) use( $ns, $birchschedule ) {
        ...
        <div class="panel-wrap birchschedule">
            <table class="form-table">
                <tr>
                    ...
                    <td>
                        <a href="<?php echo esc_attr( $calendar_address ); ?>"
                            target="_blank" >
                            <?php echo $calendar_address; ?>
                        </a>
                    </td>
                </tr>
                <tr>
...
?>
		...
		birch_defn( $ns, 'render_import_url', function( $url_id, $import_url ) use( $ns ) {
