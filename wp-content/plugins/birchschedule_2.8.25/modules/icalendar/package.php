...
birch_ns( 'birchschedule.icalendar', function( $ns ) {
		...
		birch_defn( $ns, 'export_appointments_as_ics', function( $appointments ) use ( $ns ) {
				header( 'Content-type: text/calendar; charset=utf-8' );
				echo $ns->get_appointments_as_ics( $appointments );
			} );

		birch_defn( $ns, 'export_appointment1on1s_as_ics', function( $appointment1on1s ) use ( $ns ) {
				header( 'Content-type: text/calendar; charset=utf-8' );
				echo $ns->get_appointment1on1s_as_ics( $appointment1on1s );
			} );

		birch_defn( $ns, 'get_vcalendar_properties', function() {
				return array(
					'calscale' => 'GREGORIAN',
					'method' => 'PUBLISH',
		...
		birch_defn( $ns, 'get_appointments_as_ics', function( $appointments ) use ( $ns, $birchschedule ) {

				$c = $ns->create_vcalendar();
				foreach ( $appointments as $appointment_id => $appointment ) {
					$new_appointment =
					$birchschedule->model->mergefields->get_appointment_merge_values( $appointment_id );
		...
		birch_defn( $ns, 'get_appointment1on1s_as_ics', function( $appointment1on1s ) use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'create_vcalendar', function() use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'create_vevent', function( $calendar, $appointment ) use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'merge_map', function() {

			} );

		birch_defn( $ns, 'get_appointment_summary', function( $appointment ) use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'get_appointment_summary_template', function() {
				return false;
			} );

		birch_defn( $ns, 'get_summary_separator', function() {
				return ' | ';
			} );

		birch_defn( $ns, 'get_appointment_description', function( $appointment ) use ( $ns, $birchschedule ) {
				$appointment1on1s = $appointment['appointment1on1s'];
				$description = '';
		...
		birch_defn( $ns, 'get_description_separator', function() {
				return "\n --------------------------------------------- \n";
			} );

		birch_defn( $ns, 'get_appointment1on1_description', function( $appointment1on1 ) use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'get_appointment1on1_summary', function( $appointment1on1 ) use ( $ns, $birchschedule ) {
		...
		birch_defn( $ns, 'get_appointment1on1_summary_template', function() {
				$summary = "{service_name} - {client_name}";
				return $summary;
			} );

		birch_defn( $ns, 'get_appointment1on1_location_template', function() {
				return "{location_name}";
			} );

		birch_defn( $ns, 'get_appointment1on1_description_template', function() use ( $ns ) {
				return $ns->get_appointment_export_template();
			} );

		//legacy hook - filter birchschedule_icalendar_get_appointment_export_template
		birch_defn( $ns, 'get_appointment_export_template', function() {
		...
		birch_defn( $ns, '_sanitize_value', function( $value ) {
				if ( ! is_scalar( $value ) ) {
					return $value;
				}
				$safe_eol = "\n";
